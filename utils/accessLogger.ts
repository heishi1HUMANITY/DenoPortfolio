...
const createIpv4PtrRequest = (addr: Deno.NetAddr): string => `${addr.hostname.split('.').reverse().join('.')}.in-addr.arpa`;
...
const createIpv6PtrRequest = (addr: Deno.NetAddr): string => {
  ...
  return `${segments.reverse().map(segment => Array.from(segment.padStart(4, '0')).reverse().join('.')).join('.')}.ip6.arpa`
}
